data = open('21-input.txt').read().split('\n')
...
def neighs(z, s, b):
    ...
    nexts = [z+d for d in deltas if z + d in space and 
             space[z + d] != '#' and z + d not in dists[b]]
    for y in nexts:
...
beginnings = {'s': [start],'l': [complex(w - 1, center)], 
              'd': [complex(center,0)], 'r': [complex(0,center)], 
              'u': [complex(center, h - 1)], 'dl' : [complex(w - 1, 0)], 
              'ul' : [complex(w - 1, h - 1)], 'dr': [complex(0, 0)], 
              'ur': [complex(0, h - 1)] }
...
for key in ['ur','dr','dl','ul']:
    ...
    # on trapezoids you need to go out 130 + half the length to fill that shape.   
    # You get (grids - 1) trapezoidal shapes
    total += (grids - 1) * sum([1 for z in dists[key] if 
                                dists[key][z] <= (130 + 65) and dists[key][z] % 2 == parity_trap])
    
    # on small triangles you need to go out 64 units to fill in that shape.  
    # You get (grids) number of triangle shapes
    total += grids * sum([1 for z in dists[key] if 
                          dists[key][z] <= 64 and dists[key][z] % 2 == parity_tri])
print(total)