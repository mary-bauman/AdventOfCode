...
#not written by me, written by this nice fellow
#https://pastebin.com/F1Jqe9fc#google_vignette
#although we modified a few things for part 2 bc it was a wrong answer at first
#shout out to daniel for helping w that

#I wanna rewrite this in Java at some point to furthar enhance understanding
...
def day17(filepath, part2=False):
    ...
    ends = [hl for (((x, y), (hd,hs)), hl) in v.items() if ((x, y) == (grid.height - 1, grid.width - 1))]
    if(part2):
        ends = [hl for (((x, y), (hd,hs)), hl) in v.items() if ((x, y) == (grid.height - 1, grid.width - 1) and hs>=4)]
    ans = min(ends)
    print(f"answer = {ans}, ends = {ends}")
    return ans


def main():
    inputFile = 'in.txt'
    print(f"Part 1: {day17(inputFile)}")
    print(f"Part 2: {day17(inputFile, True)}")
